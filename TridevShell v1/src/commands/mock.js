...
function startServerInstance(app, port, delay, requireKey, apiKey, dbFilePath, rl) {
    return new Promise((resolve) => {
        const server = app.listen(port, () => {
            ...
            if (requireKey && apiKey) {
                console.log(`API Key: ${chalk.green(apiKey)} (Pass via 'x-api-key' header or '?key=' query param)`);
            }
            if (delay > 0) {
        ...
        const onData = (data) => {
            ...
            if (data[0] === 8 || data[0] === 127 || data[0] === 27) {
                ...
                isPaused = !isPaused;
                if (isPaused) {
                    console.log(chalk.yellow(`\n[${new Date().toISOString()}] ⏸️ Server paused. Requests will be blocked.`));
                } else {
                    console.log(chalk.green(`\n[${new Date().toISOString()}] ▶️ Server resumed. Processing requests.`));
                }
            } else if (key === 'r') {
                if (loadDatabaseFile(dbFilePath)) {
